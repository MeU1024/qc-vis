...
const LOG_PANEL = vscode.window.createOutputChannel('Quantivine Logs');
const COMPILER_PANEL = vscode.window.createOutputChannel('Quantivine Compiler');
const STATUS_ITEM = vscode.window.createStatusBarItem(
...
function applyPlaceholders(message: string) {
  Object.entries(PLACEHOLDERS).forEach(
    ([path, placeholder]) => (message = message.replaceAll(path, placeholder))
  );
  return message;
}

function logCommandTagless(
  message: string,
  command: string,
  args: string[] = []
) {
...
function initializeStatusBarItem() {
  STATUS_ITEM.command = 'quantivine.actions';
  STATUS_ITEM.show();
...
function showErrorMessageWithExtensionLogButton(message: string) {
  const res = showErrorMessage(message, 'Open Quantivine log');
  if (res) {
    return res.then((option) => {
      switch (option) {
        case 'Open Quantivine log': {
          showLog();
          break;
        }
        default: {
